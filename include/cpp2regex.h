...
#line 71 "cpp2regex.h2"
    template <typename CharT, typename Iter, int max_groups> auto match_context<CharT,Iter,max_groups>::reverse() & -> void{std::swap(begin, end); }
...
{
    ctx.reverse();
    auto r {func(std::make_reverse_iterator(cur), ctx, true_end_func())}; 
    if (!(positive)) {
        r.matched = !(r.matched);
    }
    ctx.reverse();
