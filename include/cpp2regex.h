...
namespace regex {
    ...
    template <typename CharT, typename Iter, int max_groups, int max_alternatives, int flags> [[nodiscard]] auto match_context<CharT,Iter,max_groups,max_alternatives,flags>::get_group(auto const& group) const& -> auto { return groups[group];  }
    ...
    template <typename CharT, typename Iter, int max_groups, int max_alternatives, int flags> [[nodiscard]] auto match_context<CharT,Iter,max_groups,max_alternatives,flags>::get_group_start(auto const& group) const& -> auto { return std::distance(begin, groups[group].start); }
    template <typename CharT, typename Iter, int max_groups, int max_alternatives, int flags> [[nodiscard]] auto match_context<CharT,Iter,max_groups,max_alternatives,flags>::get_group_end(auto const& group) const& -> auto { return std::distance(begin, groups[group].end); }
    ...
    template <typename CharT, typename Iter, int max_groups, int max_alternatives, int flags> [[nodiscard]] auto match_context<CharT,Iter,max_groups,max_alternatives,flags>::get_alternative_pos(auto const& alternative) const& -> auto { return alternatives_pos[alternative];  }
    ...
    template <typename CharT, CharT Start, CharT End> [[nodiscard]] auto range_class_entry<CharT,Start,End>::includes(cpp2::in<CharT> c) -> auto { return [_0 = Start, _1 = c, _2 = End]{ return _0<=_1 && _1<=_2; }();  }
    ...
    template <typename CharT, bool negate, typename ...List> template<typename Other> [[nodiscard]] auto class_matcher_logic<CharT,negate,List...>::match(auto const& cur, auto& ctx, [[maybe_unused]] Other const& unnamed_param_3) -> auto{
        if (CPP2_UFCS(has_flag)(ctx, expression_flags::case_insensitive)) {
            if (cur!=ctx.end && (negate!=match_any<List...>(safe_tolower(*cur)) || negate!=match_any<List...>(safe_toupper(*cur)))) {
                return Other::match(cur + 1, ctx); 
            }
            else {
                return match_return(false, ctx.end); 
    ...
    template <typename Error_out> [[nodiscard]] auto regex_parser<Error_out>::next(cpp2::out<char> n) & -> bool{
        if ((pos + 1)<CPP2_UFCS(size)(regex)) {
            pos += 1;
            n.construct(regex[pos]);
            return true; 
        }
        else {
            n.construct('\0');
