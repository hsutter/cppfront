                ...
                if (CPP2_UFCS(is_simple_assignment)(expr)) 
                {
                    ...
                    if (cpp2::impl::cmp_greater(CPP2_UFCS(ssize)(additive_terms),1)) 
                    {
                        if (CPP2_UFCS(ssize)(additive_terms) != 2 
                           || CPP2_UFCS(get_op)(CPP2_ASSERT_IN_BOUNDS_LITERAL(additive_terms, 1)) != "+") 
                        {
                            CPP2_UFCS(error)(m, "temporary alpha limitation: this initial baby step only supports specifically 'a + b', not subtraction or chains of + and -");
                        }
