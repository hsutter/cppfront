...
#line 1958 "reflect.h2"
        {
            if (CPP2_UFCS(is_expression_statement)(stmt)) 
            {
                ...
                if (CPP2_UFCS(is_simple_assignment)(expr)) 
                {
                    ...
                    {
                        if (CPP2_UFCS(ssize)(additive_terms) != 2)
                        {
                            CPP2_UFCS(error)(m, "temporary alpha limitation: this does not support chains of + and -");
                        }

                        auto lhs {CPP2_UFCS(to_string)(CPP2_UFCS(get_term)(CPP2_ASSERT_IN_BOUNDS_LITERAL(additive_terms, 0)))}; 
                        if (!(CPP2_UFCS(has_parameter_or_return_named)(mf, lhs))) {
                        ...
                        auto rhs {CPP2_UFCS(to_string)(CPP2_UFCS(get_term)(CPP2_ASSERT_IN_BOUNDS_LITERAL(additive_terms, 1)))}; 
                        ...
                        auto op {CPP2_UFCS(to_string)(CPP2_UFCS(get_op)(CPP2_ASSERT_IN_BOUNDS_LITERAL(additive_terms, 1)))};

                        line1 += "" + cpp2::to_string(lhs) + "_d " + cpp2::to_string(op) + " " + cpp2::to_string(rhs) + "_d;";
                        line2 += "" + cpp2::to_string(cpp2::move(lhs)) + " " + cpp2::to_string(cpp2::move(op)) + " " + cpp2::to_string(cpp2::move(rhs)) + ";";
                    ...
                    if (cpp2::impl::cmp_greater(CPP2_UFCS(ssize)(multiplicative_terms),1)) 
                    {
                        //  TODO: Do whatever is appropriate here...
                        //  For now, just list out what we found (visible via @print)...
                        diff += "found_multiplicative_with_____";
                        auto count {0}; 
                        for ( auto const& term : cpp2::move(multiplicative_terms) ) {
                            if (cpp2::impl::cmp_greater(++count,1)) {
                                diff += CPP2_UFCS(get_op)(term) + "___";
                            }
                            diff += CPP2_UFCS(to_string)(CPP2_UFCS(get_term)(term)) + "___";
