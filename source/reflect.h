...
// 
//  Note: the ordering that should be encouraged gets the nice name
//
    ...
    -> bool
{
    ...
    for (auto& meta : n.meta_functions)
    {
        ...
        if (meta->to_string() == "interface") {
            ...
            error( "(temporary alpha limitation) unrecognized meta function name '" + meta->to_string() + "' - currently the supported names are: interface, polymorphic_base, ordered, weakly_ordered, partially_ordered" );
            return false;
        }
    }
    return true;
