    ...
    auto autodiff_context::pop_stack() & -> void{
        ...
        for ( auto const& cur : (*cpp2::impl::assert_not_null(top)).diff_request ) {
            if (!(is_in_list(cur, (*cpp2::impl::assert_not_null(top)).diff_done))) {
                // TODO: Implement declaration diff handling
                // ad: autodiff_declaration_handler = (this);
                // ad.pre_traverse(cur);
                CPP2_UFCS(error)((*cpp2::impl::assert_not_null(top)).decl, "AD: diff for `" + cpp2::to_string(CPP2_UFCS(name)(cur)) + "` in `" + cpp2::to_string(CPP2_UFCS(name)((*cpp2::impl::assert_not_null(top)).decl)) + "` requested.");
            }
        }
...
#line 5020 "reflect.h2"
        ...
        CPP2_UFCS(add_as_differentiated)(ad_ctx, cpp2::move(mf));
    }
