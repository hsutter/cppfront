...
auto hashable(meta::type_declaration& t) -> void
{
    ...
    for ( 
          ...
          auto const& o : CPP2_UFCS(get_member_objects)(t) )  { do 
    {
        ...
        if (count == 0) {
            hash += " " + cpp2::to_string(cpp2::move(o_hash.value())) + "";
        }
        else {
            hash += " ^ (" + cpp2::to_string(cpp2::move(o_hash.value())) + " << " + cpp2::to_string(count) + ")";
        }
    } while (false); ++count; }
