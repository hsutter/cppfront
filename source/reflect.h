...
#line 112 "reflect.h2"
        ...
        if (!(ret.value())) {
            error(std::string("parse failed - the source string is not a valid statement:\n" + cpp2::to_string(std::move(original_source))));
...
#line 145 "reflect.h2"
    ...
    auto compiler_services::error(cpp2::in<std::string_view> msg) const& -> void
    {
        ...
        if (!(CPP2_UFCS(empty)(metafunction_name))) {
            message = std::string("while applying @" + cpp2::to_string(metafunction_name) + " - " + cpp2::to_string(message));
        }
...
#line 168 "reflect.h2"
    auto compiler_services::report_violation(auto const& msg) const& -> void{
        error(msg);
        throw(std::runtime_error(std::string("  ==> programming bug found in metafunction @" + cpp2::to_string(metafunction_name) + " - contract violation - see previous errors")));
    }
    ...
    {                           // etc is not declared ':string_view' for compatibility with GCC 10.x
        for ( 
            ...
            CPP2_UFCS(require)(m, !(CPP2_UFCS(has_name)(m, name)), 
                       std::string("in a '" + cpp2::to_string(get_metafunction_name()) + "' type, the name '" + cpp2::to_string(name) + "' is reserved for use by the '" + cpp2::to_string(get_metafunction_name()) + "' implementation"));
        }
...
#line 957 "reflect.h2"
    ...
    if (  CPP2_UFCS(is_member_object)(m)) 
    {
        ...
        CPP2_UFCS(require)(m, !(is_default_or_numeric) || !(found_non_numeric) || CPP2_UFCS(has_name)(mo, "none"), 
            std::string(cpp2::to_string(CPP2_UFCS(name)(mo)) + ": enumerators with non-numeric values must come after all default and numeric values"));
...
#line 1039 "reflect.h2"
    ...
    CPP2_UFCS(add_member)(t, std::string("    _value            : " + cpp2::to_string(underlying_type.value()) + ";"));
    CPP2_UFCS(add_member)(t, std::string("    private operator= : (implicit out this, _val: i64) == _value = cpp2::unsafe_narrow<" + cpp2::to_string(underlying_type.value()) + ">(_val);"));
    ...
    if (bitwise) {
        ...
        CPP2_UFCS(add_member)(t, "    operator&=: ( inout this, that )                 == _value &= that._value;");
        CPP2_UFCS(add_member)(t, "    operator^=: ( inout this, that )                 == _value ^= that._value;");
        CPP2_UFCS(add_member)(t, std::string("    operator| : (       this, that ) -> " + cpp2::to_string(CPP2_UFCS(name)(t)) + "  == _value |  that._value;"));
        CPP2_UFCS(add_member)(t, std::string("    operator& : (       this, that ) -> " + cpp2::to_string(CPP2_UFCS(name)(t)) + "  == _value &  that._value;"));
        CPP2_UFCS(add_member)(t, std::string("    operator^ : (       this, that ) -> " + cpp2::to_string(CPP2_UFCS(name)(t)) + "  == _value ^  that._value;"));
    ...
    for ( auto const& e : enumerators ) {
        CPP2_UFCS(add_member)(t, std::string("    " + cpp2::to_string(e.name) + " : " + cpp2::to_string(CPP2_UFCS(name)(t)) + " == " + cpp2::to_string(e.value) + ";"));
    }

    //  Generate the common functions
    CPP2_UFCS(add_member)(t, std::string("    get_raw_value     : (this) -> " + cpp2::to_string(std::move(underlying_type.value())) + " == _value;"));
    CPP2_UFCS(add_member)(t, std::string("    operator=         : (out this) == { _value = " + cpp2::to_string(std::move(default_value)) + "._value; }"));
    CPP2_UFCS(add_member)(t, "    operator=         : (out this, that) == { }");
...
#line 1084 "reflect.h2"
    {
        ...
        for ( 
            ...
            if (e.name != "_") {// ignore unnamed values
                if (bitwise) {
                    if (e.name != "none") {
                        to_string += std::string("    if (this & " + cpp2::to_string(e.name) + ") == " + cpp2::to_string(e.name) + " { _ret += _comma + \"" + cpp2::to_string(e.name) + "\"; _comma = \", \"; }\n");
                    }
                }
                else {
                    to_string += std::string("    if this == " + cpp2::to_string(e.name) + " { return \"" + cpp2::to_string(e.name) + "\"; }\n");
                }
            }
        }
        ...
        if (bitwise) {
            ...
            to_string += std::string("    return \"invalid " + cpp2::to_string(CPP2_UFCS(name)(t)) + " value\";\n}\n");
        }
...
#line 1252 "reflect.h2"
        for ( 
            ...
            storage += comma + std::string("sizeof(" + cpp2::to_string(e.type) + ")");
        } while (false); comma = ", "; }
}
...
#line 1261 "reflect.h2"
        for ( 
              ...
              auto const& e : alternatives )  { do {
            storage += comma + std::string("alignof(" + cpp2::to_string(e.type) + ")");
        } while (false); comma = ", "; }
}
...
#line 1272 "reflect.h2"
    CPP2_UFCS(add_member)(t, std::string("    _discriminator: " + cpp2::to_string(std::move(discriminator_type)) + " = -1;\n"));

    //  Add the alternatives: is_alternative, get_alternative, and set_alternative
    for ( 
         auto const& a : alternatives ) 
    {
        CPP2_UFCS(add_member)(t, std::string("    is_" + cpp2::to_string(a.name) + ": (this) -> bool = _discriminator == " + cpp2::to_string(a.value) + ";\n"));

        CPP2_UFCS(add_member)(t, std::string("    " + cpp2::to_string(a.name) + ": (this) -> forward " + cpp2::to_string(a.type) + " pre(is_" + cpp2::to_string(a.name) + "()) = reinterpret_cast<* const " + cpp2::to_string(a.type) + ">(_storage&)*;\n"));

        CPP2_UFCS(add_member)(t, std::string("    " + cpp2::to_string(a.name) + ": (inout this) -> forward " + cpp2::to_string(a.type) + " pre(is_" + cpp2::to_string(a.name) + "()) = reinterpret_cast<*" + cpp2::to_string(a.type) + ">(_storage&)*;\n"));

        CPP2_UFCS(add_member)(t, std::string("    set_" + cpp2::to_string(a.name) + ": (inout this, _value: " + cpp2::to_string(a.type) + ") = { if !is_" + cpp2::to_string(a.name) + "() { _destroy(); std::construct_at( reinterpret_cast<*" + cpp2::to_string(a.type) + ">(_storage&), _value); } else { reinterpret_cast<*" + cpp2::to_string(a.type) + ">(_storage&)* = _value; } _discriminator = " + cpp2::to_string(a.value) + "; }\n"));
...
#line 1291 "reflect.h2"
    {
        for ( 
              auto const& a : alternatives ) {
            destroy += std::string("        if _discriminator == " + cpp2::to_string(a.value) + " { std::destroy_at( reinterpret_cast<*" + cpp2::to_string(a.type) + ">(_storage&) ); }\n");
        }
...
#line 1310 "reflect.h2"
    {
        for ( 
              auto const& a : alternatives ) {
            value_set += std::string("        if that.is_" + cpp2::to_string(a.name) + "() { set_" + cpp2::to_string(a.name) + "( that." + cpp2::to_string(a.name) + "() ); }\n");
        }
        value_set += "    }\n";
