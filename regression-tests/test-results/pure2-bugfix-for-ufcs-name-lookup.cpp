...
auto main() -> int{
  {
    int f {CPP2_UFCS(f)(t())}; 
    if (cpp2::Default.has_handler() && !(std::move(f) == 0) ) { cpp2::Default.violation(""); }
  }
  {
    auto f {t().f()}; 
    if (cpp2::Default.has_handler() && !(std::move(f) == 0) ) { cpp2::Default.violation(""); }
...
#line 33 "pure2-bugfix-for-ufcs-name-lookup.cpp2"
  if (cpp2::Default.has_handler() && !(f == 0) ) { cpp2::Default.violation(""); }
}
{
auto const& f = t().f();
...
#line 35 "pure2-bugfix-for-ufcs-name-lookup.cpp2"
  ...
  }
  {
    auto f {[](auto const& f) mutable -> auto{
      if (cpp2::Default.has_handler() && !(CPP2_UFCS(f)(t()) == 0) ) { cpp2::Default.violation(""); }
