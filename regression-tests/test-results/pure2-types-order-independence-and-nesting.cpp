...
#line 35 "pure2-types-order-independence-and-nesting.cpp2"
    auto X::exx(cpp2::in<int> count) const& -> void{
        ...
        cpp2::finally auto_37_9 {[&]() mutable -> void { std::cout << std::string("leaving call to 'why(" + cpp2::to_string(count) + ")'\n");  }}; 
...
#line 60 "pure2-types-order-independence-and-nesting.cpp2"
        template <typename T, typename U> template <int I> template<typename V, int J, typename W> auto A<T,U>::B<I>::f(W const& w) -> void { std::cout << std::string("hallo " + cpp2::to_string(w) + "\n");  }
