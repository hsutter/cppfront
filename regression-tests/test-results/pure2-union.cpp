...
[[nodiscard]] auto name_or_number::name() const& -> std::string const& { 
                                                         if (cpp2::Default.has_handler() && !(is_name()) ) { cpp2::Default.violation(""); }return *cpp2::assert_not_null(reinterpret_cast<std::string const*>(&_storage)); }
[[nodiscard]] auto name_or_number::name() & -> std::string& { 
                                                               if (cpp2::Default.has_handler() && !(is_name()) ) { cpp2::Default.violation(""); }return *cpp2::assert_not_null(reinterpret_cast<std::string*>(&_storage)); }
...
[[nodiscard]] auto name_or_number::num() const& -> cpp2::i32 const& { 
                                               if (cpp2::Default.has_handler() && !(is_num()) ) { cpp2::Default.violation(""); }return *cpp2::assert_not_null(reinterpret_cast<cpp2::i32 const*>(&_storage)); }
[[nodiscard]] auto name_or_number::num() & -> cpp2::i32& { 
                                                     if (cpp2::Default.has_handler() && !(is_num()) ) { cpp2::Default.violation(""); }return *cpp2::assert_not_null(reinterpret_cast<cpp2::i32*>(&_storage)); }
...
template <typename T> [[nodiscard]] auto name_or_other<T>::other() const& -> T const& { 
                                                 if (cpp2::Default.has_handler() && !(is_other()) ) { cpp2::Default.violation(""); }return *cpp2::assert_not_null(reinterpret_cast<T const*>(&_storage)); }
template <typename T> [[nodiscard]] auto name_or_other<T>::other() & -> T& { 
                                                       if (cpp2::Default.has_handler() && !(is_other()) ) { cpp2::Default.violation(""); }return *cpp2::assert_not_null(reinterpret_cast<T*>(&_storage)); }
