...
#line 22 "pure2-types-inheritance.cpp2"
        std::cout << std::string(cpp2::to_string(name) + " checks in for the day's shift\n");
    }

    auto Cyborg::speak() const -> void { 
        std::cout << std::string(cpp2::to_string(name) + " cracks a few jokes with a coworker\n");  }

    auto Cyborg::work() const -> void { 
        std::cout << std::string(cpp2::to_string(name) + " carries some half-tonne crates of Fe2O3 to cold storage\n");  }

    auto Cyborg::print() const& -> void { 
        std::cout << std::string("printing: " + cpp2::to_string(name) + " lives at " + cpp2::to_string(address) + "\n");  }

    Cyborg::~Cyborg() noexcept { 
        std::cout << std::string("Tired but satisfied after another successful day, " + cpp2::to_string(name) + " checks out and goes home to their family\n");  }
